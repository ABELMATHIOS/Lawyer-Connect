...
def attorneys_single():
    ...
    if form.validate_on_submit():
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO appointments (name, service, appointment_time) VALUES (?, ?, ?)", (name, service, appointment_time))
            conn.commit()
            ...
            conn.close()
    return render_template('attorneys-single.html', form=form)
...
def dashboard_l():
    if session["l_username"]:
        ...
        return render_template('lawyer-dashboard.html',detail=detail)
    else:
...
def create_table():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS appointments (
            ...
            name TEXT NOT NULL,
            service TEXT NOT NULL,  -- Added service field
            lawyer TEXT,  -- Added lawyer field
...
print("PayPal Secret:", os.getenv("PAYPAL_SECRET"))
